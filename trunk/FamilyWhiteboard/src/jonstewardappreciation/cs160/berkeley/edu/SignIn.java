    ...
    private OnClickListener customizeListen = new OnClickListener(){
    	...
    	public void onClick(View v)
		{
    		 ...
    		 if (usename.length() >= 1 &&  pswd.length() >= 1){
	    		 ...
	    		 c = db1.userLogin(usename, pswd);
	    		 db1.close();
	    		 if (c.moveToFirst())
	    		 {
	    			 Log.w("SignIn.java", "Found User");
		             startActivityForResult(myIntent, 0);
		             finish();
	    		 }
	    		 else
	    		 {
	    			 Toast.makeText(getBaseContext(), "No combination of user and password found! Please try again.", Toast.LENGTH_LONG).show();	    			 
	    		 }
    		 }else{
    			 if (usename.length() < 1){
